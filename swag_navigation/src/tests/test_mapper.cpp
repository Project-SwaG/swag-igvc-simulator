...
public:
  ...
  void map_callback(const swag_msgs::mapConstPtr& msg)
  {
  }
...
TEST_F(TestMapper, OriginCheck)
{
  ...
  const swag_msgs::map::ConstPtr& response =
      ros::topic::waitForMessage<swag_msgs::map>(map_sub.getTopic(), ros::Duration(5));
