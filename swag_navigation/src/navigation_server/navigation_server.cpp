...
#include <swag_utils/NodeUtils.hpp>
...
void NavigationServer::start(GoalHandle goal_handle)
{
  ...
  const swag_msgs::NavigateWaypointGoal &goal = *goal_handle.getGoal();
  ...
  swag_msgs::NavigateWaypointResult navigate_waypoint_result;
      ...
      !action_client_recovery_.waitForServer(connection_timeout))
  {
    ROS_ERROR_STREAM_NAMED("nav_server", "nav_server could not connect to one or more of move_base_flex actions: "
                                         "'get_path', "
                                         "'exe_path', 'recovery'!");
    navigate_waypoint_result.outcome = swag_msgs::NavigateWaypointResult ::INTERNAL_ERROR;
                                         ...
                                         const mbf_msgs::ExePathResultConstPtr &result_ptr)
{
  ...
  {
    case actionlib::SimpleClientGoalState::SUCCEEDED:
      navigate_waypoint_result.outcome = swag_msgs::NavigateWaypointResult::SUCCESS;
...
void NavigationServer::checkForOscillation(const geometry_msgs::PoseStamped &robot_pose)
{
  ...
  if (!oscillation_timeout_.isZero() && ros::Time::now() > start_time_ + oscillation_wait_time_)
  {
    ...
    double distance = swag::get_distance(robot_pose.pose.position, previous_oscillation_pose_.pose.position);
    ...
    else if (ros::Time::now() > last_oscillation_reset_ + oscillation_timeout_)
    {
      ...
      {
        swag_msgs::NavigateWaypointResult move_base_result;
        move_base_result.outcome = swag_msgs::NavigateWaypointResult::OSCILLATION;
...
void NavigationServer::runNextRecoveryBehavior(const swag_msgs::NavigateWaypointResult &navigate_waypoint_result)
{
