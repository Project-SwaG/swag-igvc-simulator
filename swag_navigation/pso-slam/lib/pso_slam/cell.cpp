...
NDTCell::NDTCell(bool init_cell_window) {
  if (init_cell_window) {
    for (unsigned int i = 0; i < PSO_WINDOW_SIZE; ++i) {
...
void PSOCell::addPoint(const Vector2d &point) {
...
bool PSOCell::build() {
  ...
  if (this->s_current_count > PSO_MAX_POINTS_PER_CELL) {
...
double PSOCell::normalDistribution(const Vector2d &point) {
  if (this->built) {
...
void PSOCell::s_calc_covar_inverse() {
