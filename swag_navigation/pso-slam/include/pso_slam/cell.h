...
#define WINDOW_INC_ID(idx) (idx = ((idx + 1) % PSO_WINDOW_SIZE))
...
public:
    T partial_terms[PSO_WINDOW_SIZE];
  ...
  void increment() {
    ...
    win_index_ = (win_index_ + 1) % PSO_WINDOW_SIZE;

    if (used_window_ < PSO_WINDOW_SIZE) {
      used_window_ += 1;
    }
...
class PSOCell {
private:
  Vector2d s_partial_sums[PSO_WINDOW_SIZE], s_current_partial_sum, s_global_sum;
  Matrix2d s_partial_covars[PSO_WINDOW_SIZE], s_global_covar_sum, s_inv_covar;
  int s_partial_counts[PSO_WINDOW_SIZE], s_current_count{0}, s_global_count{0};
  size_t s_current_window_id{0};
  inline void s_calc_covar_inverse();

public:
  std::vector<Vector2d> points_vector[PSO_WINDOW_SIZE];
  Vector2d mean;
  PSOCell(bool init_cell_window = true);
